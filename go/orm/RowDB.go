...
func (backRepoRow *BackRepoRowStruct) Init(db *gorm.DB) (Error error) {
	...
	if backRepoRow.Map_RowDBID_RowPtr != nil {
		err := errors.New("In Init, backRepoRow.Map_RowDBID_RowPtr should be nil")
		return err
	}

	if backRepoRow.Map_RowDBID_RowDB != nil {
		err := errors.New("In Init, backRepoRow.Map_RowDBID_RowDB should be nil")
		return err
	}

	if backRepoRow.Map_RowPtr_RowDBID != nil {
		err := errors.New("In Init, backRepoRow.Map_RowPtr_RowDBID should be nil")
		return err
	}

	tmp := make(map[uint]*models.Row, 0)
	backRepoRow.Map_RowDBID_RowPtr = &tmp

	tmpDB := make(map[uint]*RowDB, 0)
	backRepoRow.Map_RowDBID_RowDB = &tmpDB

	tmpID := make(map[*models.Row]uint, 0)
	backRepoRow.Map_RowPtr_RowDBID = &tmpID
...
func (backRepoRow *BackRepoRowStruct) CheckoutPhaseOne() (Error error) {
	...
	rowInstancesToBeRemovedFromTheStage := make(map[*models.Row]struct{})
	for key, value := range models.Stage.Rows {
...
func (backRepoRow *BackRepoRowStruct) RestorePhaseTwo() {
	...
	for _, rowDB := range *backRepoRow.Map_RowDBID_RowDB {
		...
		if rowDB.Element_RowsDBID.Int64 != 0 {
			rowDB.Element_RowsDBID.Int64 =
				int64(BackRepoElementid_atBckpTime_newID[uint(rowDB.Element_RowsDBID.Int64)])
		}
