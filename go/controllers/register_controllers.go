...
func (controller *Controller) onWebSocketRequestForBackRepoContent(c *gin.Context) {
	...
	if backRepo == nil {
		message := "WebSocket Connect, Stack github.com/fullstack-lang/gongmarkdown/go, Unkown stack: \"" + stackPath + "\"\n"
	...
	if err != nil {
		...
		parts := strings.Split("github.com/fullstack-lang/gongmarkdown/go", "/") // Assuming goFilePath holds the path
		component := "unknown"
		if len(parts) > 2 {
