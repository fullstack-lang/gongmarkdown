...
export class CellService {
  ...
  getCells(): Observable<CellDB[]> {
    return this.http.get<CellDB[]>(this.cellsUrl)
      .pipe(
  ...
  getCell(id: number): Observable<CellDB> {
    const url = `${this.cellsUrl}/${id}`;
    return this.http.get<CellDB>(url).pipe(
  ...
  postCell(celldb: CellDB): Observable<CellDB> {

    // insertion point for reset of pointers and reverse pointers (to avoid circular JSON)
    let _Row_Cells_reverse = celldb.Row_Cells_reverse
    celldb.Row_Cells_reverse = new RowDB

    return this.http.post<CellDB>(this.cellsUrl, celldb, this.httpOptions).pipe(
      tap(_ => {
  ...
  deleteCell(celldb: CellDB | number): Observable<CellDB> {
    const id = typeof celldb === 'number' ? celldb : celldb.ID;
    const url = `${this.cellsUrl}/${id}`;

    return this.http.delete<CellDB>(url, this.httpOptions).pipe(
  ...
  updateCell(celldb: CellDB): Observable<CellDB> {
    const id = typeof celldb === 'number' ? celldb : celldb.ID;
    const url = `${this.cellsUrl}/${id}`;
    ...
    return this.http.put<CellDB>(url, celldb, this.httpOptions).pipe(
      tap(_ => {
        // insertion point for restoration of reverse pointers
        celldb.Row_Cells_reverse = _Row_Cells_reverse
